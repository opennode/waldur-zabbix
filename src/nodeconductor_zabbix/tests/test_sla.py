...
class EventsTest(BaseMonitoringTest):
    def setUp(self):
        ...
        ResourceState.objects.create(scope=self.vm1, timestamp=timestamp, state=True)
        ResourceState.objects.create(scope=self.vm2, timestamp=timestamp, state=False)
...
class ItemTest(BaseMonitoringTest):
    ...
    def test_serializer(self):
        response = self.client.get(InstanceFactory.get_url(self.vm1))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual({'application_status': 1}, response.data['monitoring_items'])
