...
class ServiceDeletionTest(test.APITransactionTestCase):
    def setUp(self):
        ...
        self.url = factories.ZabbixServiceFactory.get_url(self.service, 'stale_services')

    def test_service_ids_are_required(self, mock_backend):
        mock_backend().get_stale_services.return_value = []
        response = self.client.delete(self.url)
        self.assertEqual(status.HTTP_400_BAD_REQUEST, response.status_code)
        self.assertTrue(mock_backend().get_stale_services.called)
